...
interface SearchBarProps {
  ...
  onVotingFilterChange: (filter: VotingFilter) => void;
  centerFilter: string;
  onCenterFilterChange: (filter: string) => void;
  centers: string[];
}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 flex-shrink-0">
        <div>
          ...
          <select
            ...
            onChange={(e) => onAffiliationFilterChange(e.target.value as AffiliationFilter)}
        ...
        <div>
          ...
          <select
            id="voting-filter"
            value={votingFilter}
            onChange={(e) => onVotingFilterChange(e.target.value as VotingFilter)}
            className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
          >
